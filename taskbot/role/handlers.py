...
from ..dao.schemas import RoleDto, RoleDtoBase
...
async def on_role_selected(call: CallbackQuery, widget, dialog_manager: DialogManager, item_id: str, **kwargs):
...
async def on_role_delete_selected(call: CallbackQuery, widget, dialog_manager: DialogManager, item_id: str, **kwargs):
    session = dialog_manager.middleware_data.get("session_without_commit")
    role_id = int(item_id)
...
async def on_role_id_input_error(message: Message, dialog_: Any, dialog_manager: DialogManager, error_: ValueError, **kwargs):
...
async def process_delete_role(call: CallbackQuery, widget, dialog_manager: DialogManager, auth: User|None, **kwargs):
    session = dialog_manager.middleware_data.get("session_with_commit")
